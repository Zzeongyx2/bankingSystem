...
import com.example.bankingsystem.Adapters.BankAccountAdapter;
...
public class OpenBankingFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle != null) {
            displayAccountDialogOnLaunch = bundle.getBoolean("DisplayOpenAccountDialog", false);
        }
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container,
        ...
        View rootView = inflater.inflate(R.layout.fragment_openbank_account, container, false);

        fab = rootView.findViewById(R.id.floating_action_btn);

        lstAccounts = rootView.findViewById(R.id.lst_openbank_accounts);
    ...
    public void displayAccountDialog() {
        ...
        accountDialog.setContentView(R.layout.openbank_account_dialog);
        ...
        // 요기
        edtAccountBank = accountDialog.findViewById(R.id.edt_payee_bank);
        edtAccountName = accountDialog.findViewById(R.id.edt_payee_name);
        edtAccountPassword = accountDialog.findViewById(R.id.edt_certification);
    ...
    private void setValues() {
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if (userProfile.getBankAccounts().size() >= 10) {
        ...
        if (userProfile.getBankAccounts().size() == 0) {
            txtTitleMessage.setText("Add an Bank Account with the button below");
            txtDetailMessage.setVisibility(View.GONE);
            lstAccounts.setVisibility(View.GONE);
        } else {
            txtTitleMessage.setText("Select an Bank Account to view Transactions");
            txtDetailMessage.setVisibility(View.VISIBLE);
            lstAccounts.setVisibility(View.VISIBLE);
        }

        BankAccountAdapter adapter = new BankAccountAdapter(this.getActivity(), R.layout.lst_openbank_accounts, userProfile.getBankAccounts());
    ...
    private void viewAccount() {
        BankTransactionFragment bankTransactionsFragment = new BankTransactionFragment();
        Bundle bundle = new Bundle();
        bundle.putInt("SelectedAccount", selectedAccountIndex);

        bankTransactionsFragment .setArguments(bundle);

        getActivity().getSupportFragmentManager().beginTransaction()
                .replace(R.id.flContent, bankTransactionsFragment ,"findThisFragment")
                .addToBackStack(null)
                .commit();
    }
    ...
    private void addAccount() {
        ...
        if (!(edtAccountBank.getText().toString().equals(""))) {
            if (!(edtAccountName.getText().toString().equals(""))) {
                if(!(edtAccountPassword.getText().toString().equals(""))) {
                    if(edtAccountPassword.getText().toString().equals("12081208")) {
                        try {
                        ...
                        if (edtAccountBank.getText().toString().length() > 10 || edtAccountName.getText().toString().length() > 10) {

                            Toast.makeText(this.getActivity(), R.string.account_name_exceeds_char, Toast.LENGTH_SHORT).show();
                            edtAccountBank.getText().clear();
                            ...
                            for (int i = 0; i < userProfile.getBankAccounts().size(); i++) {
                                if (edtAccountBank.getText().toString().equalsIgnoreCase(userProfile.getBankAccounts().get(i).getAccountBank())) {
                                    if (edtAccountName.getText().toString().equalsIgnoreCase(userProfile.getBankAccounts().get(i).getAccountName()))
                                        match = true;
                                }
                            }
                            ...
                            if (!match) {
                                ...
                                userProfile.addBankAccount(edtAccountBank.getText().toString(), edtAccountName.getText().toString(), 0);

                                if (!balance.equals("")) {
                                    if (isNum) {
                                        if (initDepositAmount >= 0.01) {
                                            userProfile.getBankAccounts().get(userProfile.getBankAccounts().size() - 1).addDepositTransaction(initDepositAmount);
                                            applicationDb.saveNewBankTransaction(userProfile, userProfile.getBankAccounts().get(userProfile.getBankAccounts().size() - 1).getAccountNo(), userProfile.getBankAccounts().get(userProfile.getBankAccounts().size() - 1).getTransactions().get(userProfile.getBankAccounts().get(userProfile.getBankAccounts().size() - 1).getTransactions().size() - 1));
                                        }
                                    }
                                }

                                applicationDb.saveNewBankAccount(userProfile, userProfile.getBankAccounts().get(userProfile.getBankAccounts().size() - 1));

                                Toast.makeText(this.getActivity(), R.string.acc_saved_successfully, Toast.LENGTH_SHORT).show();

                                if (userProfile.getBankAccounts().size() == 1) {
                                    txtTitleMessage.setText("Select an BankAccount to view BankTransactions");
                                    txtDetailMessage.setVisibility(View.VISIBLE);
                                    lstAccounts.setVisibility(View.VISIBLE);
                                }
                                ArrayList<OpenBankAccount> bankAccounts = userProfile.getBankAccounts();

                                BankAccountAdapter adapter = new BankAccountAdapter(getActivity(), R.layout.lst_openbank_accounts, bankAccounts);
                                lstAccounts.setAdapter(adapter);
                    ...
                    Toast.makeText(getActivity(), "Please enter an certification PassWord", Toast.LENGTH_SHORT).show();
                }
            } else {
                Toast.makeText(getActivity(), "Please enter an bank name", Toast.LENGTH_SHORT).show();
            }
        } else {
            Toast.makeText(getActivity(), "Please enter an bank name", Toast.LENGTH_SHORT).show();
        }
